    ...
    $.fn.fileUploader = function (config) {
        ...
        function html5Upload($form) {
            ...
            if (file) {
				...
				.success(function (data) {
				    afterEachUpload($form.attr('id'), data, 'success');
				    alert('上传成功！');
				})
				.error(function (jqXHR, textStatus, errorThrown) {
				    afterEachUpload($form.attr('id'), null, 'error', errorThrown);
				})
		...
		*/
        function beforeEachUpload($form) {
            ...
            $uploadData.find('.delete').removeClass('delete').addClass('cancel').attr('title', '取消');
        }
		...
		*/
        function toogleCancel(cancel) {
            ...
            if (cancel) {
                ...
                //Cancel Button
                $(config.buttonClear, pxButton).attr({ id: 'px-cancel', title: '取消' });
            } else {
                //Clear button
                $('#px-cancel', pxButton).attr({ id: buttonClearId, title: '清除' });
            }
        }
		...
		* Individual Upload
		*/
        $('.upload', pxUploadForm).click(function () {
        ...
        $('.delete', pxUploadForm).click(function () {
		...
		*/
        $('.cancel', pxUploadForm).click(function () {
            if (jqxhr) {
                jqxhr.abort();
            }
            ...
            if (!isHtml5) {
                $form = $(this).parents('.upload-data').children('form');
                $form.remove();
                afterEachUpload($form.attr('id'), null, 'abort', '已取消');
            }
        });
		...
		*	Cancel all uploads
		*/
        $('#px-cancel', pxButton).click(function () {
            stopUpload = true;
            if (jqxhr) {
                jqxhr.abort();
            }
            ...
            $('form', pxUploadForm).each(function () {
                afterEachUpload($(this).attr('id'), null, 'abort', '已取消');
            });
